...
package Exercises
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_b TNorCore
    ...
    Modelica.Blocks.Continuous.Integrator eneTABS(k=1/3600000)
    "Electrical energy meter with conversion to kWh"
    annotation (Placement(transformation(extent={{380,-100},{400,-80}})));
  Modelica.Blocks.Math.Add powTABS
    annotation (Placement(transformation(extent={{340,-100},{360,-80}})));
    Modelica.Blocks.Continuous.Integrator eneSurf(k=1/3600000)
    "Electrical energy meter with conversion to kWh"
    annotation (Placement(transformation(extent={{380,-134},{400,-114}})));
    Modelica.Blocks.Sources.RealExpression powSurf(y=-zonSou.slaOnGro.layMul.port_a.Q_flow
         - zonNor.slaOnGro.layMul.port_a.Q_flow)
    annotation (Placement(transformation(extent={{340,-134},{360,-114}})));
protected
  ...
  connect(powTABS.y, eneTABS.u)
    annotation (Line(points={{361,-90},{378,-90}}, color={0,0,127}));
  connect(embNor.QTot, powTABS.u1) annotation (Line(points={{61,6},{200,6},{200,
          -84},{338,-84}}, color={0,0,127}));
  connect(embSou.QTot, powTABS.u2) annotation (Line(points={{101,6},{220,6},{
          220,-96},{338,-96}}, color={0,0,127}));
  connect(powSurf.y, eneSurf.u)
    annotation (Line(points={{361,-124},{378,-124}}, color={0,0,127}));
